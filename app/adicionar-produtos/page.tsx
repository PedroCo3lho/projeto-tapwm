...
const AddProductPage = () => {
  ...
  const addProduct = async () => {
    await supabase.from('products').insert([{ nome_produto: nomeProduto, qtd_produto: qtdProduto, preco_produto: precoProduto }]);
    setNomeProduto('');
    setQtdProduto(0);
    setPrecoProduto(0);
  };
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <input
        ...
        className="border p-2 mb-2 w-full"
      />
      <button onClick={addProduct} className="bg-green-500 text-white p-2">Adicionar Produto</button>
    </div>
  );
};
